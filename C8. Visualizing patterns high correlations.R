...
correlations<-read.csv("Datos/3. Análisis de datos/DB7.Correlations_newmetric.csv", stringsAsFactors = FALSE)
...
for(i in 1:nrow(correlations)){
  ...
  corr<-round(correlations[i,5],3)
  text1<-paste("corr:",corr)
  
  ggplot(df, aes(x = df[,x_axis], y = df[,y_axis])) +
    geom_point(aes(colour = factor(class))) +
    xlab(x_axis)+ylab(y_axis)+labs(tag=text1)+theme(plot.tag.position = c(0.5,0.95))
